...
export function setHttpMode(val) {
    httpMode = val;
}
...
export function loadHlxSiteToken() {
    try {
...
export function saveHlxSiteToken(store) {
    fs.mkdirSync(TOKEN_DIR, { recursive: true });
    fs.writeFileSync(HLX_SITE_TOKEN_FILE, JSON.stringify(store, null, 2), "utf8");
}
export function clearHlxSiteToken() {
    try {
        fs.unlinkSync(HLX_SITE_TOKEN_FILE);
    }
    catch { /* ok */ }
}
let _cachedClientCredToken = null;
/**
 ...
 */
export async function getImsToken(override) {
    if (override)
        return override;
    // 1) Direct env override
    if (process.env.IMS_ACCESS_TOKEN)
        return process.env.IMS_ACCESS_TOKEN;
...
export async function refreshImsToken(clientId, refreshToken) {
    try {
        const res = await fetch(`${IMS_BASE}/ims/token/v3`, {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
...
export function getHlxToken() {
    // Env vars take priority over stored file token
    if (process.env.HLX_API_KEY)
        return process.env.HLX_API_KEY;
    if (process.env.HLX_AUTH_TOKEN)
        return process.env.HLX_AUTH_TOKEN;
    // Stored site token from da_login (aem-cli flow)
    const stored = loadHlxSiteToken();
    return stored?.token ?? undefined;
}
export async function adminRequest(method, urlPath, body, imsOverride) {
...
export async function daRequest(method, urlPath, body, contentType = "text/html", imsOverride) {
...
export function formatResult(result, successMsg) {
    if (result.ok) {
...
export function setPendingAuth(auth) {
    pendingAuth = auth;
}
...
export let _pendingLoginContext = null;
export function setPendingLoginContext(ctx) {
    _pendingLoginContext = ctx;
}
export function ensureHlxLoginServer() {
    if (_hlxLoginServerStarted)
        return;
            ...
            req.on("end", () => {
                ...
                if (!token) {
                    try {
                        ...
                        token = (j.token ?? j.siteToken ?? j.site_token ?? j.access_token ?? null);
                    }
                    catch { /* ignore */ }
                }
                if (token) {
...
export const TOOLS = [
    ...
    {
        name: "da_login",
        description: "Authenticate with Adobe via the AEM CLI login flow. Opens your browser to the Adobe login page for the specified org/site. No Adobe Developer Console credential required — uses the built-in aem-cli client ID.",
        inputSchema: {
            type: "object",
            properties: {
                org: {
    ...
    {
        name: "da_get_content",
        description: "Read the HTML source of a Document Authoring page. Returns raw HTML that can be inspected and modified.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Write new HTML content to a Document Authoring page. The content replaces the entire page source. After a successful write the page is automatically previewed via the Admin API.",
        inputSchema: {
            type: "object",
            properties: {
                org: { type: "string", description: "Adobe org / GitHub owner" },
    ...
    {
        name: "hlx_status",
        description: "Get the preview and live publish status of an AEM EDS page. Returns lastModified, sourceLocation, permissions, and links.",
        inputSchema: {
            type: "object",
            properties: {
                org: { type: "string", description: "Adobe org / GitHub owner (e.g. aemxsc)" },
    ...
    {
        name: "hlx_preview",
        description: "Trigger a preview update for a single AEM EDS page. Pulls the latest content from the authoring source (DA/SharePoint/Drive) and updates the preview environment.",
        inputSchema: {
            type: "object",
            properties: {
                org: { type: "string", description: "Adobe org / GitHub owner" },
    ...
    {
        name: "hlx_publish",
        description: "Publish an AEM EDS page to the live CDN. The page must have been previewed first.",
        inputSchema: {
            type: "object",
            properties: {
                org: { type: "string", description: "Adobe org / GitHub owner" },
    ...
    },
    {
        ...
        description: "Trigger preview updates for multiple AEM EDS pages in one job. Returns a job ID for tracking.",
        inputSchema: {
            type: "object",
            properties: {
                org: { type: "string", description: "Adobe org / GitHub owner" },
    ...
    },
    {
        ...
        description: "Publish multiple AEM EDS pages to the live CDN in one job. Returns a job ID for tracking.",
        inputSchema: {
            type: "object",
            properties: {
                org: { type: "string", description: "Adobe org / GitHub owner" },
    ...
    {
        name: "hlx_profile",
        description: "Return the authenticated user profile and available permissions for admin.hlx.page.",
        inputSchema: { type: "object", properties: {}, required: [] },
    },
    {
        name: "hlx_create_apikey",
        description: "Create a persistent API key for a site (valid 1 year). Requires admin-level auth (IMS token or existing admin key). The key value is only returned once — save it immediately into HLX_API_KEY env var.",
        inputSchema: {
            type: "object",
            properties: {
                org: { type: "string", description: "Adobe org / GitHub owner" },
 ...
 */
export async function handleTool(name, args, imsOverride) {
    const org = args.org;
    ...
    try {
        switch (name) {
            ...
            case "da_login": {
                ...
                const loginOrg = org ?? args.org;
                const loginSite = site ?? args.site;
            ...
            case "da_update_content": {
                ...
                // Auto-preview
                const previewResult = await adminRequest("POST", `/preview/${org}/${site}/${ref}${pagePath}`, undefined, imsOverride);
                const lines = [
                ...
                if (shouldPublish && previewResult.ok) {
                    const publishResult = await adminRequest("POST", `/live/${org}/${site}/${ref}${pagePath}`, undefined, imsOverride);
                    lines.push("", "--- Publish response ---", typeof publishResult.data === "string"
                        ? publishResult.data
            ...
            case "hlx_bulk_preview": {
                ...
                return {
                    content: [{
                            type: "text",
                            text: formatResult(r, r.ok ? `Bulk preview job started for ${paths.length} paths` : undefined),
                        }],
                    isError: !r.ok,
                };
            }
            case "hlx_bulk_publish": {
                ...
                return {
                    content: [{
                            type: "text",
                            text: formatResult(r, r.ok ? `Bulk publish job started for ${paths.length} paths` : undefined),
                        }],
                    isError: !r.ok,
                };
            }
            ...
            case "hlx_create_apikey": {
                const desc = args.description ?? "hlx-admin-mcp";
                const roles = args.roles ?? ["preview", "publish", "read"];
                const r = await adminRequest("POST", `/config/${org}/sites/${site}/apiKeys.json`, { description: desc, roles }, imsOverride);
                if (r.ok) {
                    const key = r.data?.key ?? "(key not in response)";
                    return {
                        content: [{
                                type: "text",
                                text: [
