...
import { SERVER_VERSION, IMS_BASE, TOOLS, handleTool, generateCodeVerifier, generateCodeChallenge, exchangeCodeForToken, getClientCredentialsToken, setHttpMode, } from "./tools.js";
// ─── Set HTTP mode flag ───────────────────────────────────────────────────────
setHttpMode(true);
// ─── Config ──────────────────────────────────────────────────────────────────
...
setInterval(() => {
    ...
    for (const [k, v] of pendingOAuthStates) {
        if (now - v.createdAt > tenMin)
            pendingOAuthStates.delete(k);
    }
...
oauthApp.get("/", (_req, res) => {
    res.send(`<html><body style="font-family:sans-serif;padding:2rem;max-width:600px">` +
...
oauthApp.get("/callback", async (req, res) => {
    const { code, state: imsState, error } = req.query;
    const pending = pendingOAuthStates.get(imsState);
    if (!pending) {
        res.status(400).send(`<html><body style="font-family:sans-serif;padding:2rem">` +
    ...
    let imsTokenData;
    try {
        imsTokenData = await exchangeCodeForToken(IMS_CLIENT_ID, code, pending.imsCodeVerifier, OAUTH_CALLBACK_URI);
    }
    catch (err) {
...
// ─── OAuth /token ─────────────────────────────────────────────────────────────
oauthApp.post("/token", async (req, res) => {
...
async function startOAuthHttpsServer() {
