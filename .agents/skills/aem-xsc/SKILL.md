...
- **Experience Workspace** (Early Access) — the next-gen DA surface: WYSIWYG canvas + AI assistant + agent collaboration. Same DA backend, same CDN pipeline. Enable per-org via `editor.path` config → see [tech-depth.md § Experience Workspace](tech-depth.md#experience-workspace)
